...
$(document).ready(function () {
    ...
    function getCityList(obj) {
        ...
        apigClient.continentListPost(params, body, additionalParams)
            .then(function (result) {

                var htmlResult = "";
                $('.datatableContent').html('');
                for (var i = 0; i < result.data.ContinentList.length; i++) {
                    htmlResult = htmlResult + "<tr>";
                    htmlResult = htmlResult + "<td class='sorting_1'>" + result.data.ContinentList[i].ContinentId + "</td>";
                    htmlResult = htmlResult + "<td class='title-width'>" + result.data.ContinentList[i].ContinentName + '</td>';
                    //htmlResult = htmlResult + '<td>' + result.data[i].StateTitle + '</td>';
                    //htmlResult = htmlResult + '<td>' + result.data[i].CountryTitle + '</td>';

                    if (!result.data.ContinentList[i].IsActive) {
                    ...
                    if (!result.data.ContinentList[i].IsPublished) {
